...
export function MembershipPage({ publicMode = false }: { publicMode?: boolean }) {
	...
	const buy = async () => {
		...
		setBuying(true);
		try {
			const response = await apiCall<ApiResponse>('POST', `/membership/buy-membership/${plan.id}`);
			if (response.success) {
			...
			error('Unable to purchase membership right now.');
		} finally {
			setBuying(false);
		}
	};
	...
	return (
		<PageShell>
			...
			<div className="grid gap-6 xl:grid-cols-[1fr_350px]">
						...
						{plans.map((p, index) => {
							...
							return (
								<button
									...
									className={`relative rounded-[28px] border p-4 text-center shadow-[0_14px_40px_rgba(47,43,35,0.05)] transition hover:-translate-y-1 ${
										active ? 'border-[#101936] bg-[#101936] text-white ring-4 ring-[#f4c95d]/30' : `${planTones[index % planTones.length]}`
									}`}
								>
									{p.popular && (
										<span className="absolute -top-3 left-1/2 -translate-x-1/2 rounded-full bg-[#f4c95d] px-3 py-1 text-[10px] font-black text-[#101936]">POPULAR</span>
									)}

									<div className={`mx-auto grid h-14 w-14 place-items-center rounded-2xl ${active ? 'bg-white/10 text-[#f4c95d]' : 'bg-white/60'}`}>
										<BadgeCheck size={25} />
									</div>

									<h3 className="mt-4 font-black">{p.name}</h3>
									<p className="mt-2 text-2xl font-black">₹{p.price}</p>

									<div className={`my-4 h-px ${active ? 'bg-white/10' : 'bg-[#eadfcd]'}`} />

									<p className={`text-xs font-black ${active ? 'text-white/55' : 'text-[#8a7653]'}`}>Daily Spins</p>
									<p className="mt-1 font-black">{p.spins} Spins</p>

									<p className={`mt-4 text-xs font-black ${active ? 'text-white/55' : 'text-[#8a7653]'}`}>21-Day Rewards</p>
				...
				<Panel className="h-fit p-5 xl:sticky xl:top-24">
					...
					<div className="mt-5 rounded-3xl border border-[#eadfcd] bg-white/60 p-4">
					...
					<button
						onClick={buy}
						disabled={buying}
						className="mt-5 w-full rounded-2xl bg-[#101936] py-4 text-sm font-black text-[#f4c95d] transition hover:bg-[#1b294f] disabled:opacity-60"
					>

...
export default function SpinPage({ onSpin }: { onSpin?: () => Promise<number> }) {
	...
	const spinNow = async () => {
		...
		try {
			const reward = onSpin ? await onSpin() : Number(VALUES[Math.floor(Math.random() * VALUES.length)].replace(/[^\d]/g, ''));
			targetIndex = segments.findIndex((segment) => Number(segment.value.replace(/[^\d]/g, '')) === reward / (onSpin ? 100 : 1));
		...
		const centerDeg = targetIndex * segmentAngle + segmentAngle / 2;
		...
		const player = disc.animate([{ transform: `rotate(${currentRotation}deg)` }, { transform: `rotate(${finalRotation}deg)` }], {
	...
	return (
		<>
			...
			>
				...
				>
					<div
						style={{ position: 'absolute', inset: '8%', borderRadius: '50%', background: 'radial-gradient(circle, rgba(251,191,36,0.2), transparent 68%)', filter: 'blur(10px)' }}
					/>
					...
					<div
						style={{
							...
							boxShadow: '0 0 35px rgba(245,158,11,0.58), 0 28px 70px rgba(0,0,0,0.24), inset 0 5px 10px rgba(255,255,255,0.55), inset 0 -12px 22px rgba(120,53,15,0.5)',
						}}
					>
						...
						>
							<div style={{ pointerEvents: 'none', position: 'absolute', inset: 0, zIndex: 20, borderRadius: '50%', background: lineGradient }} />
							...
							<div
								style={{
									...
									borderRadius: '50%',
									mixBlendMode: 'screen',
									background: 'radial-gradient(circle at 28% 18%, rgba(255,255,255,0.42), transparent 30%), linear-gradient(140deg, rgba(255,255,255,0.2), transparent 46%)',
								}}
							/>
						</div>
						...
						<button
							...
							style={{
								...
								boxShadow: '0 16px 38px rgba(0,0,0,0.42), inset 0 4px 8px rgba(255,255,255,0.48), inset 0 -8px 12px rgba(146,64,14,0.48)',
								opacity: spinning ? 0.78 : 1,
							}}
						>
							<span style={{ fontSize: 'clamp(12px, 2.1vw, 21px)', fontWeight: 950, color: '#7c3f00', userSelect: 'none' }}>{spinning ? 'WAIT' : 'SPIN'}</span>
						</button>
					</div>
				</div>
